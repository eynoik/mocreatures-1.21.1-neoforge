...
public class MoCModelKitty<T extends MoCEntityKitty> extends EntityModel<T> {
    ...
    public static final ModelLayerLocation LAYER_LOCATION = 
        new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath("mocreatures", "kitty"), "main");
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer buffer, int packedLight, int packedOverlay, int color) {
        ...
        for (int i = 0; i < 7; i++) {
            this.headParts[i].render(poseStack, buffer, packedLight, packedOverlay, color);
        }
        if (this.kittystate > 2) {
            this.headParts[7].render(poseStack, buffer, packedLight, packedOverlay, color);
        }
        if (this.kittystate == 12) {
            this.headParts[8].render(poseStack, buffer, packedLight, packedOverlay, color);
        }
        this.headParts[9].render(poseStack, buffer, packedLight, packedOverlay, color);
