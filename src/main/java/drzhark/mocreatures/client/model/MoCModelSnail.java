...
public class MoCModelSnail<T extends MoCEntitySnail> extends EntityModel<T> {
    ...
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(
            ResourceLocation.fromNamespaceAndPath("mocreatures", "snail"), "main"
    );
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer buffer, int packedLight, int packedOverlay, int color) {
        if (this.isHiding && this.type < 5) {
            this.shellDown.render(poseStack, buffer, packedLight, packedOverlay, color);
        } else {
            this.head.render(poseStack, buffer, packedLight, packedOverlay, color);
            this.antenna.render(poseStack, buffer, packedLight, packedOverlay, color);
            this.body.render(poseStack, buffer, packedLight, packedOverlay, color);
            this.shellUp.render(poseStack, buffer, packedLight, packedOverlay, color);
