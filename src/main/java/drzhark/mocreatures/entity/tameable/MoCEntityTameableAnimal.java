...
public class MoCEntityTameableAnimal extends MoCEntityAnimal implements IMoCTameable {
    ...
    protected void doBreeding() {
        ...
        for (Entity mate : list1) {
            ...
            if (!this.level().isClientSide()) {
                MoCMessageHandler.INSTANCE.send(MoCPacketDistributor.NEAR.with(() -> new MoCPacketDistributor.TargetPoint(this.getX(), this.getY(), this.getZ(), 64, this.level().dimension())), new MoCMessageHeart(this.getId()));
            }
            ...
            try {
                ...
                Mob offspring = (Mob) EntityType.byString(ResourceLocation.parse(MoCConstants.MOD_PREFIX + offspringName.toLowerCase()).toString()).map(type -> type.create(this.level())).orElse(null);
